<div style="background: #fff;" class="col-md-12">
	...
	<div class="gallery" id="links">
		...
		@endphp
		@if($value->type == 'video')
		<a class="gallery-item" href="{{$url}}" type="text/html" data-poster="{{url('/').'/media/originals/'.$value->filename}}" data-youtube="{{$my_array_of_vars['v']}}" title="{{$value->name}}" data-gallery>
			@else
			<a class="gallery-item" href="{{url('/').'/media/originals/'.$value->filename}}" title="{{$value->name}}" data-gallery>
				@endif
				<div class="image">
					@if($value->filename)
					...
					<ul class="gallery-item-controls">
						...
						<li data-preview_image="{{$value->filename}}"  data-id="{{$value->id}}"  class="edit-image">
							<span>
								<i class="fa fa-pencil"></i>
							</span>
						</li>
						<li data-id="{{$value->id}}" id="delete-image" data-box="#delete">
							<span>
