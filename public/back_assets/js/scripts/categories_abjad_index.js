$(document).ready(function() {
	$('#add-categories-abjad').on('click', function(){
		$('#modal-add-categories-abjad').modal('show');
	});
	$('.edit-category-abjad').on('click', function(){
		var base_url = window.location.origin;
		$('#get-data-categories-abjad').html("<center><img style='width:60px; margin:auto;' src='"+ base_url+ "'/back_assets/img/loading_button.gif'></center>");
		var id = $(this).data('id');
		$('#modal-edit-categories-abjad').modal('show');
		getEditCategoriesAbjad(id);
	});
	...
	function getEditCategoriesAbjad(id){
    var base_url = window.location.origin;
		var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
		$.ajax({
			type:"GET",
			url: base_url+"/backadmin/categories-abjad/"+id,
			data: {
			_token: CSRF_TOKEN,
			id :id
		},
		success: function(html){
			$('#get-data-categories-abjad').html(html);
		}
		});
		return false;
	}
	...
	$(".submit").click(function(){
		...
		$(".loading-button").css("display", 'inline-block');
			$("#form-add-categories-abjad").ajaxForm(
				{
					target: '.preview',
					beforeSend: function() {
...
var base_url = window.location.origin;
	...
	var id = $(this).data('id');
	$('#delete-id').val(id);
	$('#delete-categories-abjad').attr('action',base_url + '/backadmin/categories-abjad/' + id);
	if (box.length > 0) {
		box.toggleClass("open");
		var sound = box.data("sound");
	if (sound === 'alert')
